...
| **M. Morris Mano, *Computer System Architecture*, 3rd ed. — THE BOOK ITSELF** (archive.org item `computer-system-architecture-morris-mano-third-edition`, OCR full text, 1.1 MB, complete through ch. 12) `[added 2026-09-23]` | **1 — primary** | every end-of-chapter problem, chs. 2–9 (→ `../question-bank.md`); direct verification of any Mano claim previously resting on reproductions | `settled` for prose and problem statements; **numbers inside figures/tables need a figure check** — OCR damages subscripts and primes. Identity confirmed: problem 5-1 matches A2 Q1 word for word. Route note: only archive.org works — uobabylon.edu.iq now refuses connections, the pdfcoffee copy is a truncated preview, and the solutions manual is answers-only (no problem statements). |
